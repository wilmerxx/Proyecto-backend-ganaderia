...
public class GanadoServicioImpl implements com.grupo1.ganaderiagrupo1.Servicios.GanadoServicio {
    ...
    @Override
    public List<Double> obtenerConsumoSuplementoDetalles() {
        long totalGanado = ganadoRepositorio.count();
        Double totalSuplementoConsumido = alimentacionRepositorio.totalCantidadSuplemento();
        if (totalSuplementoConsumido == null) {
            totalSuplementoConsumido = 0.0;
        }
        double consumoPorCabeza = (totalGanado == 0) ? 0 : totalSuplementoConsumido / totalGanado;
        return Arrays.asList((double) totalGanado, totalSuplementoConsumido, consumoPorCabeza);
    }
